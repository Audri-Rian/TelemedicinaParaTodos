...
const getFriendlyFieldError = (fieldName: string): string => {
    ...
    if (normalized.includes('pelo menos') && normalized.includes('caracteres'))
        return `Use pelo menos ${rawMessage.match(/\d+/)?.[0] || '8'} caracteres em ${label}.`;
...
watch(searchTerm, (newTerm) => {
    if (newTerm.trim() === '') {
        ...
        filteredSpecializations.value = availableSpecializations.value.filter((spec) => spec.name.toLowerCase().includes(newTerm.toLowerCase()));
    }
});
...
const getSelectedSpecializations = () => {
    return availableSpecializations.value.filter((spec) => formData.value.specializations.includes(spec.id));
};
    ...
    <AuthBase title="" description="">
        ...
        <div class="relative z-10 mx-auto w-full max-w-7xl">
            <div class="grid grid-cols-1 items-stretch gap-8 lg:grid-cols-[420px_minmax(0,1fr)] lg:gap-12">
                <div
                    class="animate-fade-up rounded-[28px] bg-white/90 p-6 shadow-xl ring-1 ring-gray-200/70 backdrop-blur transition-transform duration-300 hover:-translate-y-0.5 lg:min-h-[660px]"
                >
                    <div class="mb-6">
                    ...
                    <Transition name="fade-slide">
                        <div
                            ...
                            class="mb-4 rounded-xl border border-emerald-300 bg-emerald-50 px-3 py-2 text-sm text-emerald-700"
                            role="alert"
                            aria-live="polite"
                        >
                    ...
                    <form @submit.prevent="handleSubmit" class="space-y-4">
                        <div>
                            <Label for="name" class="mb-2 block text-[11px] font-bold tracking-[0.08em] text-gray-500 uppercase">Nome completo</Label>
                            <Input
                                id="name"
                                type="text"
                                required
                                ...
                                :class="[
                                    'h-11 rounded-full border-0 bg-gray-100 px-4 text-sm shadow-inner transition-all duration-200 placeholder:text-gray-400 placeholder:transition-colors focus-visible:ring-2 focus-visible:ring-teal-500',
                            ...
                            <Transition name="fade-slide">
                                ...
                                >
                                    <span class="material-icons shrink-0 text-[14px] leading-none text-red-500" aria-hidden="true">error_outline</span
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                <Label for="crm" class="mb-2 block text-[11px] font-bold tracking-[0.08em] text-gray-500 uppercase">CRM</Label>
                                <Input
                                    id="crm"
                                    type="text"
                                    required
                                ...
                                <Transition name="fade-slide">
                                    ...
                                    >
                                        <span class="material-icons shrink-0 text-[14px] leading-none text-red-500" aria-hidden="true"
                                            >error_outline</span
                                        >{{ getFriendlyFieldError('crm') }}
                                    </p>
                                </Transition>
                            </div>

                            <div>
                                <Label for="cns" class="mb-2 block text-[11px] font-bold tracking-[0.08em] text-gray-500 uppercase"
                                    >CNS (opcional)</Label
                                >
                                <Input
                                    id="cns"
                                    ...
                                    class="h-11 rounded-full border-0 bg-gray-100 px-4 text-sm shadow-inner transition-all duration-200 placeholder:text-gray-400 placeholder:transition-colors focus-visible:ring-2 focus-visible:ring-teal-500"
                                />
                            </div>
                        </div>

                        <div>
                            <Label for="specializations" class="mb-2 block text-[11px] font-bold tracking-[0.08em] text-gray-500 uppercase"
                                >Especializações</Label
                            >
                            <div class="specialization-dropdown relative">
                                ...
                                <div
                                    ...
                                    @focusout="touchField('specializations')"
                                    tabindex="4"
                                    :class="[
                                    ...
                                    :aria-describedby="
                                        hasFieldError('specializations') && isFieldTouched('specializations') ? 'specializations-error' : undefined
                                    "
                                >
                                    <div class="flex items-center justify-between gap-2">
                                        <div class="flex min-w-0 flex-wrap items-center gap-1.5">
                                            ...
                                            <span v-if="formData.specializations.length === 0" class="text-sm text-gray-400"
                                                >Adicionar especializações</span
                                            >
                                        </div>
                                ...
                                <Transition name="dropdown-pop">
                                    <div
                                        v-if="isDropdownOpen"
                                        class="absolute z-50 mt-2 w-full overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-lg"
                                    >
                                        <div class="border-b border-gray-100 p-3">
                                            <Input
                                                v-model="searchTerm"
                                                placeholder="Buscar especializações..."
                                                class="h-9 rounded-xl border-gray-200 text-sm"
                                                @click.stop
                                            />
                                        </div>
                            ...
                            <Transition name="fade-slide">
                                ...
                                >
                                    <span class="material-icons shrink-0 text-[14px] leading-none text-red-500" aria-hidden="true">error_outline</span
                                    >{{ getFriendlyFieldError('specializations') }}
                                </p>
                            </Transition>
                        </div>

                        <div>
                            <Label for="email" class="mb-2 block text-[11px] font-bold tracking-[0.08em] text-gray-500 uppercase"
                            ...
                            <Input
                                ...
                                autocomplete="email"
                                name="email"
                                placeholder="medico@telemedicina.com"
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                <Label for="password" class="mb-2 block text-[11px] font-bold tracking-[0.08em] text-gray-500 uppercase">Senha</Label>
                                <Input
                                    id="password"
                                    type="password"
                                    required
                            ...
                            <div>
                                <Label for="password_confirmation" class="mb-2 block text-[11px] font-bold tracking-[0.08em] text-gray-500 uppercase"
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="text-xs leading-relaxed text-gray-500">
                                    <label for="terms_accepted" class="cursor-pointer">
                                        Eu aceito os
                                        <a href="/terms" target="_blank" class="font-semibold text-teal-700 hover:text-teal-800"> Termos de Uso </a>
                                        e a
                        ...
                        <div class="pt-1">
                            ...
                            <div
                                v-if="rateLimit.remainingAttempts < 2 && rateLimit.remainingAttempts > 0"
                                id="rate-limit-warning"
                                class="mt-2 flex items-center justify-center gap-1 text-center text-sm text-orange-600"
                                role="alert"
                                aria-live="polite"
                            >
                ...
                <div class="animate-fade-left relative hidden h-full min-h-[660px] rounded-[28px] p-6 lg:block">
                    <div class="absolute inset-0 overflow-hidden rounded-[28px]">
                        <div class="absolute top-24 -left-10 h-40 w-40 rounded-3xl bg-white/20"></div>
                    ...
                    <div class="relative flex h-full items-end justify-center">
                        ...
                        <div class="absolute right-8 bottom-8 left-8 rounded-3xl bg-white/90 p-6 shadow-lg backdrop-blur">
                            <p class="text-3xl leading-tight font-extrabold text-gray-900">O futuro da medicina é humano.</p>
                            <p class="mt-2 text-base text-gray-600">Sua jornada para uma prática clínica mais eficiente e conectada começa aqui.</p>
                        </div>
                    </div>
                </div>
            </div>
