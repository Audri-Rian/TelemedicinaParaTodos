...
import { AlertCircle, Calendar, CheckCircle2, Clock3, Loader2, MapPin, Pencil, Plus, Trash2 } from 'lucide-vue-next';
...
const summaryState = ref<Summary>(
    props.summary ?? {
        next_session: null,
...
const handleSubmit = async () => {
    ...
    try {
        if (modalMode.value === 'create') {
            ...
            await axios.put(availabilityRoutes.update.url({ doctor: doctorId.value, slot: selectedSlot.value.id }), payload);
...
<template>
    <div class="flex flex-1 flex-col gap-6">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div class="space-y-1.5">
                <p class="text-sm font-medium tracking-wide text-sky-600 uppercase">Gestão de horários</p>
                <h1 class="text-3xl font-semibold text-slate-900">Disponibilidade do médico</h1>
                <p class="text-sm text-slate-500">
                    Visualize e ajuste seus horários específicos sem misturar com a agenda recorrente. Horários passados ficam bloqueados apenas para
                    consulta.
                </p>
            </div>
            ...
            <Card class="border-slate-200 shadow-sm">
                <CardHeader>
                    ...
                    <CardTitle class="text-2xl text-slate-900">
                        ...
                        <template v-else> Sem sessões agendadas </template>
                    </CardTitle>
                </CardHeader>
                <CardContent class="space-y-3 text-sm text-slate-600">
                    <p>
                        {{
                            summaryState.next_session?.patient_name ? `Paciente: ${summaryState.next_session.patient_name}` : 'Sem paciente vinculado'
                        }}
                    </p>
                    <div class="rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
                        <p class="text-xs tracking-wide text-slate-500 uppercase">Próximos 7 dias</p>
                        <div class="mt-1 flex items-center justify-between text-sm font-semibold text-slate-800">
                            ...
                            <span class="text-emerald-600"> {{ summaryState.next_seven_days.available }} disponíveis </span>
                        </div>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card class="border-slate-200 shadow-sm">
                ...
                <CardContent class="space-y-3 text-sm text-slate-600">
                    ...
                    <div class="rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2">
                        <p class="text-xs tracking-wide text-emerald-600 uppercase">Disponíveis esta semana</p>
            ...
            <Card class="border-slate-200 shadow-sm">
                ...
                <CardContent class="space-y-3 text-sm text-slate-600">
                    <div class="space-y-2">
                        ...
                        >
                            <div>
                                <p class="font-semibold text-slate-800">{{ session.time }} · {{ session.date }}</p>
        ...
        <Card class="border-slate-200 shadow-md">
            <CardHeader class="flex flex-col gap-3 border-b border-slate-100 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <CardDescription> Monitoramento de {{ windowLabel }} — Horários passados são exibidos em cinza para consulta. </CardDescription>
                </div>
                    ...
                    <div class="flex rounded-full border border-slate-200 bg-slate-50 p-1">
                        <button
                            ...
                            :class="[
                                ...
                                viewMode === filter.value ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500',
                            ]"
                        >
                            {{ filter.label }}
                        </button>
                    </div>
                    <div class="flex items-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs text-slate-500">
                        <span class="h-2 w-2 rounded-full bg-emerald-500"></span>
                        Disponível
                        <span class="ml-3 h-2 w-2 rounded-full bg-amber-500"></span>
                        Ocupado
                        <span class="ml-3 h-2 w-2 rounded-full bg-slate-400"></span>
                        Passado
                    </div>
                </div>
            </CardHeader>
            <CardContent class="space-y-6">
                ...
                <div
                    v-if="!filteredTimeline.length && !isRefreshing"
                    class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-slate-500"
                >
                ...
                <div v-for="day in filteredTimeline" :key="day.date" class="space-y-4">
                    <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        ...
                        <span :class="['text-xs font-semibold tracking-wide uppercase', day.is_past ? 'text-slate-400' : 'text-sky-600']">
                            ...
                            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                                <div class="flex flex-1 items-start gap-4">
                                    ...
                                    <div class="space-y-1">
                                        <p class="text-lg font-semibold text-slate-900">
                                            {{ slot.time_range }}
                                            <span class="text-sm font-normal text-slate-500"> · {{ slot.duration_minutes }} min </span>
                                        </p>
                                ...
                                <div class="flex flex-col gap-3 md:items-end">
                                    ...
                                    >
                                        <span class="h-2 w-2 rounded-full" :class="statusStyles[slot.status.code]?.dot || 'bg-slate-400'"></span>
                                        {{ statusStyles[slot.status.code]?.text || slot.status.label }}
                                    </span>

                                    <div class="flex flex-wrap gap-2">
                                        <Button size="sm" variant="outline" :disabled="!slot.can_edit || isSubmitting" @click="openEditModal(slot)">
        ...
        <Card class="border-slate-200 bg-gradient-to-br from-white to-slate-50">
            ...
            <CardContent class="grid gap-4 text-sm text-slate-600 md:grid-cols-3">
                <div class="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                    <p class="text-xs font-semibold tracking-wide text-sky-600 uppercase">Edição segura</p>
                    <p class="mt-2 text-slate-700">
                        Apenas horários futuros ficam habilitados para edição ou exclusão. Passados permanecem apenas para consulta.
                    </p>
                </div>
                <div class="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                    <p class="text-xs font-semibold tracking-wide text-sky-600 uppercase">Feedback em tempo real</p>
                    <p class="mt-2 text-slate-700">
                        Toda ação exibe alertas verdes de sucesso ou vermelhos em caso de erro, além de spinner durante carregamentos.
                    </p>
                </div>
                <div class="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                    <p class="text-xs font-semibold tracking-wide text-sky-600 uppercase">Modal moderno</p>
                    <p class="mt-2 text-slate-700">
    ...
    <!-- Botão flutuante -->
    <Button class="fixed right-6 bottom-6 shadow-xl" size="lg" @click="openCreateModal">
        <Plus class="h-4 w-4" />
        Novo horário
    </Button>
    ...
    <Dialog :open="isModalOpen" @update:open="(value) => (!value && !isSubmitting ? closeModal() : null)">
        <DialogContent class="rounded-2xl sm:max-w-lg">
            <DialogHeader>
                ...
                <DialogDescription> Defina a data, horário e local da disponibilidade. Apenas horários futuros podem ser salvos. </DialogDescription>
            </DialogHeader>

            <form class="space-y-4" @submit.prevent="handleSubmit">
                <div class="grid gap-2">
                    <Label for="availability_date">Data</Label>
                    <Input id="availability_date" type="date" v-model="formState.date" required />
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="grid gap-2">
                        ...
                        <Input id="availability_start" type="time" v-model="formState.startTime" required />
                        <InputError :message="formErrors.start_time?.[0]" />
                    </div>
                    <div class="grid gap-2">
                        <Label for="availability_end">Hora fim</Label>
                        <Input id="availability_end" type="time" v-model="formState.endTime" required />
                ...
                <div class="grid gap-2">
                    ...
                    <Select id="availability_location" v-model="formState.locationId">
                        <option value="">Selecionar posteriormente</option>
                        <option v-for="location in locationsState" :key="location.id" :value="location.id">
                ...
                <DialogFooter class="flex flex-col gap-3 sm:flex-row sm:justify-end">
                    <Button variant="outline" type="button" @click="closeModal" :disabled="isSubmitting"> Cancelar </Button>
