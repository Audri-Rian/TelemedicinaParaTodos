...
function runSfuTest(config) {
  ...
  async function enumerateDevices() {
    try {
      ...
      const populate  = (sel, list, label) => {
        ...
        sel.innerHTML = list.map((d, i) =>
          `<option value="${d.deviceId}">${d.label || `${label} ${i + 1}`}</option>`
        ).join('');
    ...
    } catch (e) { log('Erro ao enumerar dispositivos', e?.message, 'warn'); }
  }
  ...
  async function join() {
    ...
    const makeTransport = async (direction) => {
      ...
      const t  = direction === 'send'
        ? device.createSendTransport({ id: d.id, iceParameters: d.iceParameters, iceCandidates: d.iceCandidates, dtlsParameters: d.dtlsParameters })
        : device.createRecvTransport ({ id: d.id, iceParameters: d.iceParameters, iceCandidates: d.iceCandidates, dtlsParameters: d.dtlsParameters });
      t.on('connect', async ({ dtlsParameters }, cb, eb) => {
        try { await request('connectWebRtcTransport', { transportId: t.id, dtlsParameters }); cb(); }
        catch (e) { eb(e); }
      });
      if (direction === 'send') {
  ...
  function leave(reason = 'user') {
    ...
    if (ws?.readyState === WebSocket.OPEN) { send({ action: 'leave' }); ws.close(); }
    ws = null;
  ...
  function connect() {
    ...
    ws.onopen = () => {
      ...
      join().catch(e => { log('Join falhou', e.message, 'error'); connected = false; updateUI(); });
    };
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (!msg.action && msg.data?.producerClosed && msg.data?.consumerId) {
          ...
          if (consumer) { consumer.close(); consumers.delete(msg.data.consumerId); lastConsumerStats.delete(msg.data.consumerId); }
    ...
    ws.onclose = () => {
      ...
      if (!intentionalClose && autoReconnect) {
        ...
        setTimeout(() => { ws = null; connect(); }, 2000);
      }
    };
  ...
  async function startCamera(videoOnly = false) {
    if (!sendTransport) { log('startCamera: sem sendTransport'); return; }
    try {
      ...
      if (localVideo) { localVideo.muted = true; localVideo.srcObject = localStream; localVideo.play().catch(() => {}); }
  ...
  async function toggleCamera() {
    if (cameraOn) {
      ...
      if (localStream) localStream.getVideoTracks().forEach(t => { t.stop(); createdTracks.delete(t); localStream.removeTrack(t); });
      cameraOn = false; videoSendPaused = false; producerSFUPaused = false;
      ...
      try {
        ...
        videoProducer = await sendTransport.produce({ track, codecOptions: { videoGoogleStartBitrate: 1000 }, appData: { source: 'camera' } });
        cameraOn = true; videoSendPaused = false; producerSFUPaused = false;
        log('Câmera ligada', { id: videoProducer.id });
      } catch (e) { log('Erro ao ligar câmera', e?.message, 'error'); }
    }
    updateUI();
  }
  ...
  async function toggleBlackVideo() {
    if (!videoProducer) { log('blackVideo: sem videoProducer ativo'); return; }
    try {
      ...
      if (localStream) {
        localStream.getVideoTracks().forEach(t => { if (t !== newTrack) { t.stop(); createdTracks.delete(t); localStream.removeTrack(t); } });
        localStream.addTrack(newTrack);
      }
      if (localVideo) localVideo.srcObject = localStream;
      log('Vídeo substituído por canvas preto (simula falha de câmera)');
    } catch (e) { log('Erro vídeo preto', e?.message, 'error'); }
  }
  ...
  async function switchCamera() {
    ...
    } catch (e) { log('Erro ao trocar câmera', e?.message, 'error'); }
  }
  ...
  async function switchAudioInput() {
    ...
    try {
      ...
      if (localStream) {
        localStream.getAudioTracks().forEach(t => { if (t !== newTrack) { t.stop(); createdTracks.delete(t); localStream.removeTrack(t); } });
        localStream.addTrack(newTrack);
      }
      log('Microfone trocado (replaceTrack)', { deviceId });
    } catch (e) { log('Erro ao trocar microfone', e?.message, 'error'); }
  }
  ...
  async function switchAudioOutput() {
    ...
    for (const entry of remotePeerContainers.values()) {
      if (entry.audioEl?.setSinkId) {
        try { await entry.audioEl.setSinkId(deviceId); count++; } catch (e) { log('setSinkId erro', e?.message, 'warn'); }
      }
    }
  ...
  async function pauseProducerSFU() {
    ...
    try {
      ...
      updateUI();
    } catch (e) { log('pauseProducer erro', e?.message, 'error'); }
  }
  ...
  async function resumeProducerSFU() {
    ...
    try {
      ...
      updateUI();
    } catch (e) { log('resumeProducer erro', e?.message, 'error'); }
  }
  ...
  async function pauseAllConsumersSFU() {
    ...
    for (const [consumerId] of consumers) {
      try { await request('pauseConsumer', { consumerId }); count++; } catch {}
    }
  ...
  async function resumeAllConsumersSFU() {
    let count = 0;
    for (const [consumerId] of consumers) {
      try { await request('resumeConsumer', { consumerId }); count++; } catch {}
    }
    consumersSFUPaused = false;
  ...
  function testCleanup() {
    ...
    log(`  ● Ativas (live): ${activeTracks.length}`, activeTracks.map(t => `${t.kind}/${t.label?.slice(0, 20)}`));
    ...
    if (typeof performance !== 'undefined' && performance.memory) {
      ...
      log(`  ● Heap: usado=${mb(performance.memory.usedJSHeapSize)} / total=${mb(performance.memory.totalJSHeapSize)} / limite=${mb(performance.memory.jsHeapSizeLimit)}`);
    }
    log('────────────────────────────────────────────');
  ...
  async function consume(producerId, peerId, kind) {
    ...
    try {
      ...
      const consumer = await recvTransport.consume({ id: data.id, producerId: data.producerId, kind: data.kind, rtpParameters: data.rtpParameters, type: data.type });
      ...
      if (kind === 'video') {
        ...
        kf(); [500, 1500, 3000].forEach(ms => setTimeout(kf, ms));
      } else {
      ...
      updateUI();
    } catch (e) { log('Consume erro', e?.message ?? String(e), 'error'); }
  }
  ...
  function getOrCreateRemotePeerContainer(peerId) {
    ...
    audioEl.autoplay = true; audioEl.muted = false; audioEl.volume = 1;
  ...
  async function refreshStats() {
    ...
    if (videoProducer && typeof videoProducer.getStats === 'function') {
      try {
        ...
        let bytesSent = 0, pkts = 0, frames = 0, rtt = null, loss = null;
        let width = 0, height = 0, fps = null;
        report.forEach(s => {
        ...
        rows.push({
          label: '📹 Vídeo (send)', kbps, rtt, loss: loss ?? null,
          fps: fps != null ? fps.toFixed(0) : null,
          res: (width && height) ? `${width}×${height}` : null, jitter: null, state,
        });
      } catch {}
    }
    ...
    if (audioProducer && typeof audioProducer.getStats === 'function') {
      try {
        ...
        let bytesSent = 0, rtt = null, loss = null;
        report.forEach(s => {
        ...
        rows.push({ label: '🎤 Áudio (send)', kbps, rtt, loss: loss ?? null, fps: null, res: null, jitter: null, state: micOn ? '✓' : 'MUDO' });
      } catch {}
    }
    ...
    for (const [consumerId, consumer] of consumers) {
      try {
        ...
        let bytes = 0, lost = 0, rcvd = 0, jitterSec = 0, fps = null, width = 0, height = 0, rtt = null;
        report.forEach(s => {
  ...
  // ── Cleanup all ───────────────────────────────────────────────────────────
  function cleanupAll() { leave('cleanup'); log('Limpeza completa.'); }
  ...
  btnActivateAudio?.addEventListener('click', () => {
    remotePeerContainers.forEach(entry => {
      ...
      entry.audioEl.muted = false; entry.audioEl.volume = 1;
      entry.audioEl.play().catch(() => {});
    });
  ...
  window.addEventListener('beforeunload', () => { if (connected) send({ action: 'leave' }); });
