            ...
            const handler = (e) => {
                try {
                    ...
                    if (msg.ok) {
                        ...
                        reject(new Error(msg.error?.message || 'Erro'));
                    }
                } catch (err) {
                    ws.removeEventListener('message', handler);
                    reject(err);
    ...
    async function refreshStats() {
        ...
        if (audioProducer && typeof audioProducer.getStats === 'function') {
            try {
                ...
                let bytes = 0;
                report.forEach((s) => {
                    if (s.type === 'outbound-rtp') bytes += s.bytesSent || 0;
                });
                let kbps = '—';
    ...
    async function startMedia() {
        try {
            ...
            if (audioTrack) {
                ...
                if (infoMic) {
                    infoMic.textContent = 'ON';
                    infoMic.style.color = '#4ade80';
                }
            ...
            if (videoTrack) {
                ...
                if (infoCam) {
                    infoCam.textContent = 'ON';
                    infoCam.style.color = '#4ade80';
                }
