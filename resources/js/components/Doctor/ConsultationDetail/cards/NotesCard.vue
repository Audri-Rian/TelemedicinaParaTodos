...
<template>
    <CollapsibleCard
        ...
        kbd="⌘6"
        :hint="!locked ? 'Observações privadas do médico' : undefined"
        :locked="locked"
        :collapsed="collapsed"
        @toggle="emit('toggle')"
    >
        ...
        <template v-else>
            <div class="cp-field-wrap">
                <textarea
                    v-model="model"
                    class="cp-field"
                    placeholder="Hipóteses adicionais, raciocínio, considerações para próximas consultas…"
                    rows="4"
                    maxlength="5000"
                    @input="emit('change')"
                />
            ...
            <div class="cp-field-foot">
                <span />
                <span class="char-count" :class="{ 'is-warn': model.length / 5000 > 0.9 }"> {{ model.length.toLocaleString('pt-BR') }} / 5.000 </span>
            </div>
        </template>
    </CollapsibleCard>
</template>
