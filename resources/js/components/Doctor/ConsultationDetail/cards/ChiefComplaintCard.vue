...
<template>
    <CollapsibleCard
        id="chief-complaint-card"
        num="1"
        title="Queixa principal"
        kbd="⌘1"
        :hint="!locked ? 'Anotação subjetiva' : undefined"
        :locked="locked"
        :collapsed="collapsed"
        @toggle="emit('toggle')"
    >
        ...
        <template v-else>
            <div class="cp-field-wrap">
                <textarea
                    v-model="model"
                    class="cp-field"
                    placeholder="O que o paciente relata? Use as palavras dele(a) sempre que possível."
                    rows="3"
                    maxlength="1000"
                    @input="emit('change')"
                />
            ...
            <div class="cp-field-foot">
                <span />
                <span class="char-count" :class="{ 'is-warn': model.length / 1000 > 0.9 }"> {{ model.length.toLocaleString('pt-BR') }} / 1.000 </span>
            </div>
        </template>
    </CollapsibleCard>
</template>
