    ...
    <SidebarGroup class="px-2 py-0">
        <SidebarMenu>
            <template v-for="item in items" :key="item.title">
                <SidebarMenuItem v-if="!item.children?.length" :data-tour="item.tourId">
                ...
                <SidebarMenuItem v-else class="group/nested" :data-tour="item.tourId">
                    <Collapsible
                    ...
                    >
                        <div class="flex w-full min-w-0 items-stretch gap-0 rounded-md">
                            <SidebarMenuButton as-child class="min-w-0 flex-1 pr-1" :is-active="false" :tooltip="item.title">
                            ...
                            <CollapsibleTrigger as-child>
                                <button
                                    type="button"
                                    class="inline-flex size-8 shrink-0 items-center justify-center rounded-md text-sidebar-foreground/70 outline-hidden transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 focus-visible:ring-sidebar-ring"
                        ...
                        <CollapsibleContent>
                            <SidebarMenuSub class="mt-0.5 ml-3 gap-0.5 border-l border-dashed border-sidebar-border/60 py-1 pl-5">
                                <SidebarMenuSubItem v-for="child in item.children" :key="child.title">
                                    <SidebarMenuSubButton
                                        as-child
                                        size="sm"
                                        class="h-8 px-2 text-[13px] font-normal text-muted-foreground hover:text-sidebar-foreground"
                                        :is-active="isCurrentRoute(child.href)"
                                    >
                                        <Link :href="child.href" class="flex w-full min-w-0 items-center gap-2.5">
                                            <component :is="child.icon" v-if="child.icon" class="size-3.5 shrink-0 opacity-60" />
