...
- Implementar videoconferência em tempo real com WebRTC via SFU MediaSoup; Laravel Reverb fica responsável pelos eventos de negócio da chamada.
...
- Consultas por videoconferência em tempo real com WebRTC via SFU MediaSoup e eventos de negócio via Laravel Reverb.
...
- Consultas online com videoconferência em qualidade aceitável de áudio e vídeo (WebRTC/SFU MediaSoup).
...
- **Complexidade da videoconferência:** dependência de WebRTC/SFU MediaSoup, WebSocket do SFU e Laravel Reverb para eventos; falhas de rede, NAT ou firewall podem afetar chamadas.
...
O sistema deve permitir solicitação e realização de chamada de vídeo entre médico e paciente em tempo real com WebRTC via SFU MediaSoup. O Laravel Reverb deve notificar estados de negócio da chamada; a sinalização técnica de mídia ocorre no WebSocket do SFU.  
Atores: Médico, Paciente.  
Prioridade: Essencial.  
Critério de aceitação: `Call` e `Room` criadas para a consulta ou chamada ad-hoc; token JWT emitido pelo Laravel; áudio e vídeo estabelecidos via SFU; estados de disponibilidade, aceite, recusa e encerramento propagados aos participantes.
...
O sistema adota uma **arquitetura de microserviços** organizada em camadas. O **serviço de aplicação principal** é implementado em um deploy Laravel (PHP), com fronteira clara entre backend e frontend via Inertia.js (SPA híbrida, sem API REST dedicada para a interface principal). Nesse serviço residem autenticação web, prontuário, agendamento e orquestração dos demais componentes.

A plataforma **é composta por microserviços especializados**, executados em processos ou containers independentes e acoplados por contratos (HTTP, filas, WebSocket, armazenamento de objetos), entre eles: SFU de videoconferência (MediaSoup), mensageria assíncrona (RabbitMQ), cache e sessão (Redis), objetos (MinIO/S3), tempo real (Laravel Reverb) e API REST para parceiros. Internamente, o serviço Laravel mantém **camadas lógicas**: Migrations → Models (Eloquent) → Services (lógica de negócio) → Controllers (HTTP) → Events/Observers/Jobs; no frontend: páginas (Inertia), layouts, componentes UI e composables. Diagrama e fluxos em `docs/layers/architecture-governance/Architecture/Arquitetura.md` e `docs/DistributedSystems/Contexto.md`.
...
- **Events/Observers/Jobs:** Eventos de domínio (`VideoCallAvailable`, `VideoCallRequested`, `VideoCallAccepted`, `VideoCallRejected`, `VideoCallEnded`, `AppointmentStatusChanged`, etc.), observers (AppointmentsObserver), jobs (`AutoStartVideoCall`, `EndScheduledVideoCalls`, `EndZombieVideoCalls`, `GenerateMedicalRecordPDF`); broadcasting via Laravel Reverb.
...
- Vue.js 3 (Composition API), TypeScript, Inertia.js (Vue 3 adapter), Vite 7; Tailwind CSS 4, Reka UI, Lucide Vue, VueUse, mediasoup-client (videoconferência SFU). Versões em `package.json` (ex.: vue ^3.5.13, tailwindcss ^4.1.1, @inertiajs/vue3 ^2.1.0).
...
- **Laravel Reverb:** Servidor WebSocket para eventos de negócio em tempo real (videochamada, mensagens, notificações).
- **MediaSoup / SFU:** Servidor de mídia para WebRTC; o frontend usa `mediasoup-client` e token JWT emitido pelo Laravel.
...
MEDIASOUP. mediasoup - Cutting Edge WebRTC Video Conferencing. Disponível em: https://mediasoup.org/. Acesso em: 02 jun. 2026.
