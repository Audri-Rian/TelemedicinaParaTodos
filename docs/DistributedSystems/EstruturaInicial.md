...
> Atualização 2026-05-16: a arquitetura operacional de homelab foi consolidada no **PC1**. A fonte de verdade atual é [`deploy/pc1/`](../../deploy/pc1/) e a spec [`docs/specs/infraestrutura-pc1-consolidada.md`](../specs/infraestrutura-pc1-consolidada.md). A distribuição PC1/PC2/PC3 abaixo permanece como histórico e referência de decisões anteriores.

## Arquitetura atual consolidada (PC1)

O PC1 passa a hospedar a stack local completa: Laravel/PHP-FPM, Nginx, PostgreSQL, Redis, RabbitMQ, MinIO, Reverb, workers e scheduler. A exposição pública deve acontecer somente por Cloudflare Tunnel no host PC1, entregando `app.<dominio>` e `ws.<dominio>` para `http://127.0.0.1:80`. O SFU MediaSoup permanece no Hetzner e é acessado pelo Laravel via `SFU_HTTP_URL` / `SFU_WS_URL`, preferencialmente pela rede Tailscale.

Para deploy, use [`deploy/pc1/README.md`](../../deploy/pc1/README.md). Antes de subir a stack consolidada, verifique conflitos com containers antigos, especialmente MinIO em `9000/9001`.
