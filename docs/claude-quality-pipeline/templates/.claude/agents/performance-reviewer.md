...
description: Revisor de performance focado em Laravel 12 + Inertia + Vue 3. Analisa arquivos staged ou modificados buscando gargalos típicos do ecossistema — queries N+1 em Eloquent, over-fetching, falta de índices, cache ausente ou mal invalidado, jobs que deveriam ir pra fila RabbitMQ, Inertia props pesadas, re-renders Vue desnecessários, assets não otimizados. Invoque ANTES de commitar código que toca Controllers, Services, Repositórios, Models com scopes, Jobs, Events/Listeners, componentes Vue com listas grandes ou queries pesadas.
